        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.categorias.update', $categoria->slug) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="descripcion" id="descripcion" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $categoria->descripcion) }}</textarea>
                            @error('descripcion')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        {{-- CAMPO IMAGEN ACTUAL Y NUEVA IMAGEN --}}
                        <div class="mb-4">
                            ...
                            @if ($categoria->imagen)
                                <div class="mt-2 mb-4">
                                    ...
                                    <img src="{{ Storage::url($categoria->imagen) }}" alt="{{ $categoria->nombre }}" class="h-24 w-24 object-cover rounded-lg shadow-md">
                                </div>
                            @endif
                            ...
                            @error('imagen')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                            <p class="text-xs text-gray-500 mt-1">Sube una nueva imagen para reemplazar la actual. Déjalo vacío para mantener la imagen existente.</p>
                        </div>
