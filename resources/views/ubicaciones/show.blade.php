                        ...
                        <div class="bg-oaxaca-card-bg rounded-xl shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105 group border border-oaxaca-primary border-opacity-10"> {{-- Fondo de tarjeta, sombra y borde sutil --}}
                            ...
                            <div class="p-5"> {{-- Padding ajustado --}}
                                <h3 class="text-2xl font-display font-semibold mb-2 text-oaxaca-primary leading-tight"> {{-- Título de artesanía con font-display y color primario --}}
                                    <a href="{{ route('artesanias.show', $artesania->slug) }}" class="hover:text-oaxaca-secondary transition-colors">{{ $artesania->nombre }}</a>
                                </h3>
                                ...
                                @if ($artesania->categoria)
                                    <p class="text-oaxaca-text-dark text-sm mt-1">Categoría: <a href="{{ route('categorias.show', $artesania->categoria->slug) }}" class="font-semibold text-oaxaca-accent hover:underline">{{ $artesania->categoria->nombre }}</a></p> {{-- Categoría con enlace y color de acento --}}
                                @endif
                                ...
                                <a href="{{ route('artesanias.show', $artesania->slug) }}" class="mt-auto inline-block w-full bg-oaxaca-primary text-white px-6 py-3 rounded-lg hover:bg-oaxaca-secondary transition-colors text-center text-lg font-medium shadow-md transform hover:scale-105"> {{-- Botón con colores primarios y hover --}}
                                    Ver Detalles
                                </a>
                            </div>
                        </div>
