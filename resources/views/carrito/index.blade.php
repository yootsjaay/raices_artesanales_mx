...
<div class="container mx-auto py-8 px-4 max-w-7xl">
    <h1 class="text-5xl md:text-6xl font-display font-bold text-oaxaca-primary mb-10 text-center animate-fade-in">Tu Carrito de Compras</h1>

    {{-- Mensajes de sesión --}}
    @if (session('success'))
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-6 font-sans" role="alert">
    ...
    @if (session('error'))
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6 font-sans" role="alert">
    ...
    @if ($cartItems->isEmpty())
        {{-- Carrito vacío --}}
        <div class="bg-oaxaca-card-bg p-10 rounded-xl shadow-lg text-center border border-oaxaca-primary border-opacity-10">
            ...
            <p class="text-2xl text-oaxaca-text-dark mb-6 font-sans">Tu carrito está vacío. ¡Explora nuestras auténticas artesanías!</p>
            <a href="{{ route('artesanias.index') }}" class="inline-block bg-oaxaca-primary hover:bg-oaxaca-secondary text-oaxaca-text-light font-semibold px-8 py-4 rounded-lg transition-colors duration-300 shadow-md transform hover:scale-105">
    ...
    @else
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 bg-oaxaca-card-bg p-6 rounded-xl shadow-lg border border-oaxaca-primary border-opacity-10">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-oaxaca-primary divide-opacity-20">
                        <thead class="bg-oaxaca-primary bg-opacity-5">
                            <tr>
                                <th class="px-6 py-3 text-left text-sm font-semibold text-oaxaca-primary uppercase tracking-wider">Producto</th>
                        ...
                        <tbody class="bg-oaxaca-card-bg divide-y divide-oaxaca-primary divide-opacity-10">
                            @foreach ($cartItems as $item)
                                @php
                                    // Determinar la imagen a mostrar (de la variante o de la artesanía)
                                    $itemImage = asset('storage/images/artesanias/placeholder-alebrije.jpg'); // Imagen por defecto
                                    
                                    if ($item->artesania_variant && is_array($item->artesania_variant->imagen_variant) && !empty($item->artesania_variant->imagen_variant[0])) {
                                        $itemImage = asset('storage/' . $item->artesania_variant->imagen_variant[0]);
                                    } elseif ($item->artesania && is_array($item->artesania->imagen_artesanias) && !empty($item->artesania->imagen_artesanias[0])) {
                                        $itemImage = asset('storage/' . $item->artesania->imagen_artesanias[0]);
                                    }

                                    // Determinar el nombre y detalles
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <img class="h-24 w-24 rounded-lg object-cover border border-oaxaca-accent border-opacity-30"
                                                src="{{ $itemImage }}"
                                                alt="{{ $itemName }}">

                                            <div class="ml-4">
                                                <a href="{{ route('artesanias.show', ['slug' => $item->artesania->slug, 'variant' => $item->artesania_variant->id ?? null]) }}"
                                                    class="text-base font-semibold text-oaxaca-primary hover:text-oaxaca-secondary transition-colors">
                                                    {{ $itemName }}
                                                </a>
                                                @if($variantDetails)
                                    ...
                                    <td class="px-6 py-4">
                                        <form action="{{ route('carrito.actualizar') }}" method="POST" class="flex items-center">
                                            ...
                                            <input type="number"
                                                ...
                                                min="0"
                                                max="{{ $availableStock }}"
                                                class="w-24 p-2 border border-oaxaca-primary border-opacity-30 rounded-md shadow-sm focus:ring-oaxaca-tertiary focus:border-oaxaca-tertiary text-oaxaca-text-dark text-base"
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <form action="{{ route('carrito.remover') }}" method="POST">
                                            ...
                                            @method('DELETE')
                                            <input type="hidden" name="id" value="{{ $item->id }}">
                                            <button type="submit" class="text-oaxaca-secondary hover:text-red-700 transition-colors">Eliminar</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div class="mt-8 text-right">
                    <form action="{{ route('carrito.vaciar') }}" method="POST" class="inline-block">
                        @csrf
                        <button type="submit" class="bg-oaxaca-accent bg-opacity-10 text-oaxaca-accent px-6 py-3 rounded-lg hover:bg-oaxaca-accent hover:text-white transition-colors duration-300 text-base font-semibold shadow-md">
                            Vaciar Carrito
                        </button>
                    </form>
                </div>
            ...
            {{-- Resumen del pedido --}}
            <div class="bg-oaxaca-card-bg shadow-lg rounded-xl p-6 flex flex-col justify-between border border-oaxaca-primary border-opacity-10">
                <div>
                    <h2 class="text-2xl font-display font-semibold text-oaxaca-primary mb-5">Resumen del Pedido</h2>
                    <ul class="divide-y divide-oaxaca-primary divide-opacity-10 mb-6">
                        @foreach($cartItems as $item)
                            @php
                            ...
                            <li class="py-3 flex items-center gap-4">
                                ...
                                <span class="font-bold text-oaxaca-primary">${{ number_format($item->quantity * $item->price, 2) }} MXN</span>
                            </li>
                        @endforeach
                    </ul>

                    <div class="border-t border-oaxaca-primary border-opacity-20 pt-4 mt-4">
                        <div class="flex justify-between font-bold text-xl mb-4 text-oaxaca-primary">
                            <span>Total:</span>
